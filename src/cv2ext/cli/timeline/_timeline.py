...
def timeline_cli() -> None:
    ...
    img_size: tuple[int, int] | None = tuple(map(int, args.img_size.strip("[]").split(","))) if args.img_size is not None else None  # type: ignore[assignment]
    max_imgsize = 2
    ...
    if args.bboxes is not None:
        ...
        with bboxespath.open() as f:
            try:
                bboxes = [tuple(map(int, line.strip().split(","))) for line in f.readlines()]  # type: ignore[misc]
